...
No third-party community ONNX weights are used. The graph is exported from the official checkpoint, wrapped to expose the stable `image` input and `depth` `[1,1,392,518]` output contract, then constant-folded for the fixed input shape without changing model output values.
